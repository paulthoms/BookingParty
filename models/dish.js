...
function DishDAO(MysqlDB) {
    ...
    this.postDishRestaurantIDAndDishIDModel = function (dish, user, callback) {
        ...
        var sql = "insert into Dish (Name,Image,Description,Restaurant_ID) values (?,?,?,?)";
        MysqlDB.query(sql, [dish.Name, dish.Image, dish.Description, user.user.Restaurant_ID], function (error, results) {
            if (error) {
                callback({
                    "status": "error",
                    "code": "400",
